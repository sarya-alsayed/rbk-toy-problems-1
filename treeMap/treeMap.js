     ...
     for (var i=0;i<this.children.length;i++){
        if(this.children[i].children.length===0)
          ...
          for (var j=0;j<this.children[i].children.length;j++){
           this.children[i].children[j].prototype.map(callback);
...
Tree.prototype.mapInPlace = function (callback) {
  ...
  var newTree=new tree(this.value);
  ...
  if (newTree.children.length===0){
    callback(newTree.value);
     ...
     for (var i=0;i<newTree.children.length;i++){
        if(newTree.children[i].children.length===0)
          callback(newTree.children[i].value);
        else{
          for (var j=0;j<newTree.children[i].children.length;j++){
           newTree.children[i].children[j].prototype.mapInPlace(callback);
